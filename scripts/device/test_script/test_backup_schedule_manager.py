...
#end these all should be run using python exe
#how to stop schedule task in python
#readme with open source format
#best logging lib with UI
...
def load_env_variables():
    ...
    if is_all_env_values_provided == True:
        logging.info('Provided device information : IP address: %s user name: %s password: %s device type: %s secret: %s',
                      ip_address, username, password, device_type, secret)
    else:
        logging.warning('Please provided valid device information to proceed the backup schedule task.')


def create_backup_file():
    time_now = datetime.datetime.now().replace(microsecond=0)
    if is_all_env_values_provided == False:
        logging.warning('Backup schedule task terminated due to in correct device information.')
        logging.warning('Please update correct device information in env file.')
    else:
        DEVICE = {
                    'ip': ip_address,
                    'username': username,
