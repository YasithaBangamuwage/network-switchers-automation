...
def create_cisco_backup_folder(switch_folder_name):
    if not os.path.exists(
        get_directory_path() + "/Backups/cisco/" + switch_folder_name
    ):
        os.makedirs(get_directory_path() + "/Backups/cisco/" + switch_folder_name)
        logger.info(
            "CISCO Backup Scheduler created backup folder called : %s.",
            switch_folder_name,
        )
...
def create_cisco_backup_file(switch_backup_folder_name, backup_file_name, backup_data):
    try:
        filepath = os.path.join(
            get_directory_path() + "/Backups/cisco/" + switch_backup_folder_name,
            backup_file_name,
        )
        backup_file = open(filepath + ".txt", "w")
        backup_file.write(backup_data)
        logger.info(
            "CISCO Backup Scheduler created backup file successfully. %s",
            backup_file_name,
        )
        backup_file.close()
    except Exception:
        logger.exception(
            "CISCO Backup Scheduler error occurred while backup file creation"
        )
    finally:
        backup_file.close()
...
def generate_cisco_backups(cisco_switches):
    ...
    for cisco_switch in cisco_switches:
        ...
        switch_backup_folder_name = (
            cisco_switch["tag_name"] + "_" + cisco_switch["ip_address"]
        )
        ...
        try:
            ...
            logger.info(
                "CISCO Backup Scheduler connecting to (IP address : %s) at %s",
                cisco_switch["ip_address"],
                str(time_now),
            )
