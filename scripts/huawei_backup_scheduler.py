...
def create_huawei_backup_folder(switch_folder_name):
    if not os.path.exists(
        get_directory_path() + "/Backups/huawei/" + switch_folder_name
    ):
        os.makedirs(get_directory_path() + "/Backups/huawei/" + switch_folder_name)
        logger.info(
            "HUAWEI Backup Scheduler created backup folder called : %s.",
            switch_folder_name,
        )
...
def create_huawei_backup_file(switch_backup_folder_name, backup_file_name, backup_data):
    try:
        filepath = os.path.join(
            get_directory_path() + "/Backups/huawei/" + switch_backup_folder_name,
            backup_file_name,
        )
        backup_file = open(filepath + ".txt", "w")
        backup_file.write(backup_data)
        logger.info(
            "HUAWEI Backup Scheduler created backup file successfully. %s",
            backup_file_name,
        )
        backup_file.close()
    except Exception:
        logger.exception(
            "HUAWEI Backup Scheduler error occurred while backup file creation"
        )
    finally:
        backup_file.close()
...
def generate_huawei_backups(huawei_switches):
    ...
    for huawei_switch in huawei_switches:
        ...
        switch_backup_folder_name = (
            huawei_switch["tag_name"] + "_" + huawei_switch["ip_address"]
        )
        ...
        try:
            ...
            logger.info(
                "HUAWEI Backup Scheduler connecting to (IP address : %s) at %s",
                huawei_switch["ip_address"],
                str(time_now),
            )
            ...
            backup_file_time_stamp = datetime.datetime.now().strftime(
                "%Y_%m_%d-%I_%M_%S_%p"
            )
            backup_file_name = switch_backup_folder_name + "_" + backup_file_time_stamp
            create_huawei_backup_file(
                switch_backup_folder_name, backup_file_name, show_run_output
            )

        except NetMikoTimeoutException:
            logger.exception(
                "Device not reachable (Please contact system administrator)"
            )
        except NetMikoAuthenticationException:
            logger.exception(
                "Authentication Failure (Please contact system administrator)"
            )
        except SSHException:
            logger.exception(
                "Make sure SSH is enabled (Please contact system administrator)"
            )

    next_backup_file_time_stamp = datetime.datetime.now() + timedelta(weeks=4)
    logger.info(
        "HUAWEI Backup Scheduler Next backup files will be created at: %s",
